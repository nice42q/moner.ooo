<?php
$l_lang_meta         = "es";
$l_rtl               = "false";
$l_page_title        = "Conversión de XMR a EUR/BTC/CHF/USD y mucho más";
$l_meta_description  = "El tipo de cambio en directo de Monero en muchas monedas diferentes, gratis para todos.";
$l_meta_keywords     = "Monero, XMR, fiat, valor, tasa, en directo, cambio, conversión";

$l_title_h1          = "conversión a";
$l_fiatSelect        = "Elegir moneda";
$l_fiatInput         = "Campo de entrada de valor fiduciario";
$l_xmrInput          = "Campo de entrada del valor de Monero";
$l_clipboard_copy_tooltip = "Copiar al portapapeles";
$l_copytoclipboard_success_text = "¡Copiado!";
$l_copytoclipboard_error_text   = "¡Error!";

$l_refresh_info_minute  = "Los valores se mueven cada minuto.";
$l_refresh_info_minutes = "Los valores se actualizan cada %s minutos.";
$l_refresh_info_seconds = "Los valores se actualizan cada %s segundos.";
$l_last_at              = "Última actualización:";
$l_hr_min_sec           = "Horas:Minutos:Segundos (hh:mm:ss)";
$l_unixTime             = "Tiempo Unix:";

$l_aed = "Dírham de los Emiratos Árabes Unidos";
$l_ars = "Peso argentino";
$l_aud = "Dólar australiano";
$l_bdt = "Taka bangladesí";
$l_bhd = "Dinar de Bahréin";
$l_bmd = "Dólar de Bermudas";
$l_brl = "Real brasileño";
$l_btc = "Bitcoin";
$l_cad = "Dólar canadiense";
$l_chf = "Franco suizo";
$l_clp = "Peso chileno";
$l_cny = "Renminbi";
$l_czk = "Corona checa";
$l_dkk = "Corona danesa";
$l_eth = "Ethereum";
$l_eur = "Euro";
$l_gbp = "Libra esterlina";
$l_gel = "Lari georgiano";
$l_hkd = "Dólar de Hong Kong";
$l_huf = "Forinto húngaro";
$l_idr = "Rupia indonesia";
$l_ils = "Nuevo séquel israelí";
$l_inr = "Rupia india";
$l_jpy = "Yen";
$l_krw = "El won surcoreano";
$l_kwd = "Dinar de Kuwait";
$l_lkr = "Rupia de Sri Lanka";
$l_ltc = "Litecoin";
$l_mmk = "Kyat birmano";
$l_mxn = "Peso mexicano";
$l_myr = "Ringgit malayo";
$l_ngn = "Naira";
$l_nok = "Corona noruega";
$l_nzd = "Dólar neozelandés";
$l_php = "Peso filipino";
$l_pkr = "Rupia paquistaní";
$l_pln = "Esloti";
$l_rub = "Rublo ruso";
$l_sar = "Rial saudí";
$l_sek = "Corona sueca";
$l_sgd = "Dólar de Singapur";
$l_thb = "Bat tailandés";
$l_try = "Lira turca";
$l_twd = "Nuevo dólar de Taiván";
$l_uah = "Hryvnia";
$l_usd = "Dólar estadounidense";
$l_vef = "Bolívar venezolano";
$l_vnd = "Vietnamita Đồng";
$l_xag = "Plata (onza fina)";
$l_xau = "Oro (onza fina)";
$l_zar = "Rand sudafricano";

$l_info_main         = "Los valores de este sitio web son meramente informativos.&nbsp;El valor no está garantizado y está sujeto a cambios sin previo aviso.";

$l_data_provided     = "Datos proporcionados por";
$l_cg_url            = "https://www.coingecko.com/es/coins/monero";
$l_cg_url_hreflang    = "es";
$l_coingecko         = $l_data_provided . " <a class=\"text-white\" href=\"$l_cg_url\" hreflang=\"$l_cg_url_hreflang\" rel=\"external\" target=\"_blank\">CoinGecko</a>.";

$l_getmonero         = '<a class="text-white" href="https://www.getmonero.org/es/" hreflang="es" target="_blank" rel="external">Página oficial</a> | <a class="text-white" href="https://ccs.getmonero.org/" hreflang="en" target="_blank" rel="external">CCS</a> | <a class="text-white" href="https://www.monero.observer/resources/" hreflang="en" target="_blank" rel="external">Explorador</a> | <a class="text-white" href="https://www.monerotalk.live/" hreflang="en" target="_blank" rel="external">Monero Talk</a>';
$l_countrymonero     = '&nbsp;| <a class="text-white" href="https://t.me/MoneroEsp" hreflang="en" target="_blank" rel="external">Telegram ES</a>';

$l_donation_page     = "página de donaciones";
$l_kuno_homepage_url = "https://kuno.anne.media/lang/es/";
$l_kuno_homepage_url_hreflang = "es";
$l_kuno_text         = "Kuno – Recaudar fondos con Monero";
$l_webserver_sponsor = "Webhosting proporcionado por";

$l_api_update        = $l_last_at . " <u title=\"$l_hr_min_sec\"><span id=\"lastUpdateDisplay\">00:00:00</span></u>, <span id=\"displayTimeZone\">hora local</span>.";

$l_info              = $l_info_main."&nbsp;<span id=\"refreshInfo\" class=\"mb-1\">--</span>&nbsp;$l_api_update&nbsp;$l_coingecko<br><a target=\"_blank\" href=\"https://kuno.anne.media/donate/onml/\" rel=\"external\" hreflang=\"en\"><img loading=\"lazy\" align=\"middle\" src=\"./img/kuno-monero-26x26.png\" width=\"17\" height=\"17\" alt=\"Kuno - Moner.ooo $l_donation_page\"></a>&nbsp;<a target=\"_blank\" href=\"$l_kuno_homepage_url\" class=\"text-white\" rel=\"external\" hreflang=\"$l_kuno_homepage_url_hreflang\">$l_kuno_text</a> | <a class=\"text-white\" href=\"https://github.com/nice42q/moner.ooo\" hreflang=\"en\" rel=\"external\" target=\"_blank\">GitHub</a> | <a style=\"text-decoration:none; font-weight:bold;\" class=\"text-white\" href=\"https://servers.guru/\" hreflang=\"en\" rel=\"external\" target=\"_blank\">$l_webserver_sponsor<img loading=\"lazy\" align=\"middle\" src=\"./img/servers-guru.svg\" height=\"19\" alt=\"Servers Guru\" title=\"Servers Guru\"></a>";