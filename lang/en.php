<?php
$l_lang_meta         = "en";
$l_rtl               = "false";
$l_page_title        = "XMR conversion to EUR/BTC/CHF/USD and many more";
$l_meta_description  = "The Monero live exchange rate in many different currencies, free for everyone.";
$l_meta_keywords     = "Monero, XMR, fiat, value, rate, live, exchange, conversion";

$l_title_h1          = "conversion to";
$l_fiatSelect        = "Currency choice";
$l_fiatInput         = "Fiat value input field";
$l_xmrInput          = "Monero value input field";
$l_clipboard_copy_tooltip = "Copy to clipboard";
$l_copytoclipboard_success_text = "Copied!";
$l_copytoclipboard_error_text   = "Error!";

$l_refresh_info_minute  = "The values are updated every minute.";
$l_refresh_info_minutes = "The values are updated every %s minutes.";
$l_refresh_info_seconds = "The values are updated every %s seconds.";
$l_last_at              = "Last at";
$l_hr_min_sec           = "Hours:Minutes:Seconds (hh:mm:ss)";
$l_unixTime             = "Unix Time:";

$l_aed = "UAE Dirham";
$l_ars = "Argentine Peso";
$l_aud = "Australian Dollar";
$l_bdt = "Bangladeshi Taka";
$l_bhd = "Bahraini Dinar";
$l_bmd = "Bermudan Dollar";
$l_brl = "Brazilian Real";
$l_btc = "Bitcoin";
$l_cad = "Canadian Dollar";
$l_chf = "Swiss Franc";
$l_clp = "Chilean Peso";
$l_cny = "Renminbi";
$l_czk = "Czech Koruna";
$l_dkk = "Danish Krone";
$l_eth = "Ethereum";
$l_eur = "Euro";
$l_gbp = "Pound sterling";
$l_gel = "Georgian Lari";
$l_hkd = "Hong Kong Dollar";
$l_huf = "Hungarian Forint";
$l_idr = "Indonesian Rupiah";
$l_ils = "Israeli New Shekel";
$l_inr = "Indian Rupee";
$l_jpy = "Yen";
$l_krw = "South Korean won";
$l_kwd = "Kuwaiti Dinar";
$l_lkr = "Sri Lankan Rupee";
$l_ltc = "Litecoin";
$l_mmk = "Myanmar Kyat";
$l_mxn = "Mexican Peso";
$l_myr = "Malaysian Ringgit";
$l_ngn = "Nigerian Naira";
$l_nok = "Norwegian Krone";
$l_nzd = "New Zealand Dollar";
$l_php = "Philippine Peso";
$l_pkr = "Pakistani Rupee";
$l_pln = "Złoty";
$l_rub = "Russian Ruble";
$l_sar = "Saudi Riyal";
$l_sek = "Swedish Krona";
$l_sgd = "Singapore Dollar";
$l_thb = "Baht";
$l_try = "Turkish Lira";
$l_twd = "New Taiwan Dollar";
$l_uah = "Hryvnia";
$l_usd = "US Dollar";
$l_vef = "Venezuelan bolívar";
$l_vnd = "Vietnamese dong";
$l_xag = "Silver (troy ounce)";
$l_xau = "Gold (troy ounce)";
$l_zar = "South African Rand";

$l_info_main         = "The exchange rates on this site are for information purposes only.&nbsp;They are not guaranteed to be accurate and are subject to change without notice.";

$l_data_provided     = "Data provided by";
$l_cg_url            = "https://www.coingecko.com/en/coins/monero";
$l_cg_url_hreflang    = "en";
$l_coingecko         = $l_data_provided . " <a class=\"text-white\" href=\"$l_cg_url\" hreflang=\"$l_cg_url_hreflang\" rel=\"external\" target=\"_blank\">CoinGecko</a>.";

$l_getmonero         = '<a class="text-white" href="https://www.getmonero.org/" hreflang="en" target="_blank" rel="external">Official website</a> | <a class="text-white" href="https://ccs.getmonero.org/" hreflang="en" target="_blank" rel="external">CCS</a> | <a class="text-white" href="https://www.monero.observer/resources/" hreflang="en" target="_blank" rel="external">Observer</a> | <a class="text-white" href="https://www.monerotalk.live/" hreflang="en" target="_blank" rel="external">Talk</a>';
$l_countrymonero     = '&nbsp;| <a class="text-white" href="https://t.me/monero" hreflang="en" target="_blank" rel="external">Telegram XMR</a> | <a class="text-white" href="https://www.revuo-xmr.com/" hreflang="en" target="_blank" rel="external">Revuo Monero</a>';

$l_donation_page     = "donation page";
$l_kuno_homepage_url = "https://kuno.anne.media/";
$l_kuno_homepage_url_hreflang = "en";
$l_kuno_text         = "Kuno – Fundraise with Monero";
$l_webserver_sponsor = "Webhosting provided by";

$l_api_update        = $l_last_at . " <u title=\"$l_hr_min_sec\"><span id=\"lastUpdateDisplay\">00:00:00</span></u>, <span id=\"displayTimeZone\">local time</span>.";

$l_info              = $l_info_main."&nbsp;<span id=\"refreshInfo\" class=\"mb-1\">--</span>&nbsp;$l_api_update&nbsp;$l_coingecko<br><a target=\"_blank\" href=\"https://kuno.anne.media/donate/onml/\" rel=\"external\" hreflang=\"en\"><img loading=\"lazy\" align=\"middle\" src=\"./img/kuno-monero-26x26.png\" width=\"17\" height=\"17\" alt=\"Kuno - Moner.ooo $l_donation_page\"></a>&nbsp;<a target=\"_blank\" href=\"$l_kuno_homepage_url\" class=\"text-white\" rel=\"external\" hreflang=\"$l_kuno_homepage_url_hreflang\">$l_kuno_text</a> | <a class=\"text-white\" href=\"https://github.com/nice42q/moner.ooo\" hreflang=\"en\" rel=\"external\" target=\"_blank\">GitHub</a> | <a style=\"text-decoration:none; font-weight:bold;\" class=\"text-white\" href=\"https://servers.guru/\" hreflang=\"en\" rel=\"external\" target=\"_blank\">$l_webserver_sponsor<img loading=\"lazy\" align=\"middle\" src=\"./img/servers-guru.svg\" height=\"19\" alt=\"Servers Guru\" title=\"Servers Guru\"></a>";