<?php
$l_lang_meta         = "ar";
$l_rtl               = "true";
$l_page_title        = "تحويل XMR إلى EUR/BTC/CHF/USD والكثير غير ذلك";
$l_meta_description  = "سعر الصرف المباشر للمونيرو بالعديد من العملات المختلفة.. مجاني للجميع.";
$l_meta_keywords     = "مونرو، XMR، فيات، قيمة، سعر، مباشر، تبادل، تحويل";

$l_title_h1          = "التحويل إلى";
$l_fiatSelect        = "اختيار العملة";
$l_fiatInput         = "حقل إدخال قيمة الفيات";
$l_xmrInput          = "حقل إدخال قيمة مونرو";
$l_clipboard_copy_tooltip = "نسخ إلى الحافظة";
$l_copytoclipboard_success_text = "تم النسخ!";
$l_copytoclipboard_error_text   = "خطأ!";

$l_refresh_info_minute  = "يتم تحديث القيم كل دقيقة.";
$l_refresh_info_minutes = "يتم تحديث القيم كل %s دقائق.";
$l_refresh_info_seconds = "يتم تحديث القيم كل %s ثوانٍ.";
$l_last_at              = "آخر تحديث عند الساعة";
$l_hr_min_sec           = "الساعات:الدقائق:الثواني (س س:د د:ث ث)";
$l_unixTime             = "وقت يونكس:";

$l_aed = "الدرهم الإماراتي";
$l_ars = "البيزو الأرجنتيني";
$l_aud = "الدولار الاسترالي";
$l_bdt = "التاكا البنجلاديشية";
$l_bhd = "الدينار البحريني";
$l_bmd = "الدولار البرمودي";
$l_brl = "الريال البرازيلي";
$l_btc = "البيتكوين";
$l_cad = "الدولار الكندي";
$l_chf = "الفرنك السويسري";
$l_clp = "البيزو التشيلي";
$l_cny = "الرنمينبي";
$l_czk = "الكورونا التشيكية";
$l_dkk = "الكرونة الدنماركية";
$l_eth = "الإيثيريوم";
$l_eur = "اليورو";
$l_gbp = "الجنيه الاسترليني";
$l_gel = "اللاري الجورجي";
$l_hkd = "دولار هونج كونج";
$l_huf = "الفورنت المجري";
$l_idr = "الروبية الإندونيسية";
$l_ils = "الشيكل الإسرائيلي الجديد";
$l_inr = "الروبية الهندية";
$l_jpy = "الين";
$l_krw = "الوون الكوري الجنوبي";
$l_kwd = "الدينار الكويتي";
$l_lkr = "الروبية السريلانكية";
$l_ltc = "اللايتكوين";
$l_mmk = "كيات ميانمار";
$l_mxn = "البيزو المكسيكي";
$l_myr = "الرينغيت الماليزي";
$l_ngn = "النيرة النيجيرية";
$l_nok = "الكرونة النرويجية";
$l_nzd = "الدولار النيوزلندي";
$l_php = "البيزو الفلبيني";
$l_pkr = "الروبية الباكستانية";
$l_pln = "زلوتي";
$l_rub = "الروبل الروسي";
$l_sar = "الريال السعودي";
$l_sek = "الكرونة السويدية";
$l_sgd = "الدولار السنغافوري";
$l_thb = "باهت";
$l_try = "الليرة التركية";
$l_twd = "الدولار التايواني الجديد";
$l_uah = "هريفنا";
$l_usd = "الدولار الأمريكي";
$l_vef = "بوليفار فنزويلي";
$l_vnd = "دونج فيتنامي";
$l_xag = "الفضة (الأونصة طروادة)";
$l_xau = "الذهب (الأونصة طروادة)";
$l_zar = "الراند الجنوب أفريقي";

$l_info_main         = "أسعار الصرف على هذا الموقع هي لأغراض إعلامية فقط.&nbsp;لا نضمن دقتها، وهي عرضة للتغيير دون إشعار.";

$l_data_provided     = "البيانات مقدمة من";
$l_cg_url            = "https://www.coingecko.com/ar/%D8%B9%D9%85%D9%84%D8%A7%D8%AA/monero";
$l_cg_url_hreflang    = "ar";
$l_coingecko         = $l_data_provided . " <a class=\"text-white\" href=\"$l_cg_url\" hreflang=\"$l_cg_url_hreflang\" rel=\"external\" target=\"_blank\">CoinGecko</a>.";

$l_getmonero         = '<a class="text-white" href="https://www.getmonero.org/ar/" hreflang="ar" target="_blank" rel="external">الموقع الرسمي</a> | <a class="text-white" href="https://ccs.getmonero.org/" hreflang="en" target="_blank" rel="external">نظام التمويل (CCS)</a> | <a class="text-white" href="https://www.monero.observer/resources/" hreflang="en" target="_blank" rel="external">Observer</a> | <a class="text-white" href="https://www.monerotalk.live/" hreflang="en" target="_blank" rel="external">Talk</a>';
$l_countrymonero     = '';

$l_donation_page     = "صفحة التبرعات";
$l_kuno_homepage_url = "https://kuno.anne.media/lang/ar/";
$l_kuno_homepage_url_hreflang = "ar";
$l_kuno_text         = "Kuno – جمع التبرعات باستخدام Monero";
$l_webserver_sponsor = "استضافة الويب مقدمة من";

$l_api_update        = $l_last_at . " <u title=\"$l_hr_min_sec\"><span id=\"lastUpdateDisplay\">00:00:00</span></u>، <span id=\"displayTimeZone\">التوقيت المحلي</span>.";

$l_info              = $l_info_main."&nbsp;<span id=\"refreshInfo\" class=\"mb-1\">--</span>&nbsp;$l_api_update&nbsp;$l_coingecko<br><a target=\"_blank\" href=\"https://kuno.anne.media/donate/onml/\" rel=\"external\" hreflang=\"en\"><img loading=\"lazy\" align=\"middle\" src=\"./img/kuno-monero-26x26.png\" width=\"17\" height=\"17\" alt=\"Kuno - Moner.ooo $l_donation_page\"></a>&nbsp;<a target=\"_blank\" href=\"$l_kuno_homepage_url\" class=\"text-white\" rel=\"external\" hreflang=\"$l_kuno_homepage_url_hreflang\">$l_kuno_text</a> | <a class=\"text-white\" href=\"https://github.com/nice42q/moner.ooo\" hreflang=\"en\" rel=\"external\" target=\"_blank\">GitHub</a> | <a style=\"text-decoration:none; font-weight:bold;\" class=\"text-white\" href=\"https://servers.guru/\" hreflang=\"en\" rel=\"external\" target=\"_blank\">$l_webserver_sponsor<img loading=\"lazy\" align=\"middle\" src=\"./img/servers-guru.svg\" height=\"19\" alt=\"Servers Guru\" title=\"Servers Guru\"></a>";