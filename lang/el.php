<?php
$l_lang_meta         = "el";
$l_rtl               = "false";
$l_page_title        = "XMR μετατροπή σε EUR/BTC/CHF/USD και πολλά άλλα";
$l_meta_description  = "Η ζωντανή ισοτιμία του Monero σε πολλά διαφορετικά νομίσματα, δωρεάν για όλους.";
$l_meta_keywords     = "Monero, XMR, fiat, αξία, ισοτιμία, ζωντανά, ανταλλαγή, μετατροπή";

$l_title_h1          = "μετατροπή σε";
$l_fiatSelect        = "Επιλογή νομίσματος";
$l_fiatInput         = "Πεδίο εισαγωγής αξίας fiat";
$l_xmrInput          = "Πεδίο εισαγωγής αξίας Monero";
$l_clipboard_copy_tooltip = "Αντιγραφή στο πρόχειρο";
$l_copytoclipboard_success_text = "Αντιγράφηκε!";
$l_copytoclipboard_error_text   = "Σφάλμα!";

$l_refresh_info_minute  = "Οι τιμές ενημερώνονται κάθε λεπτό.";
$l_refresh_info_minutes = "Οι τιμές ενημερώνονται κάθε %s λεπτά.";
$l_refresh_info_seconds = "Οι τιμές ενημερώνονται κάθε %s δευτερόλεπτα.";
$l_last_at              = "Τελευταία ενημέρωση στις";
$l_hr_min_sec           = "Ώρες:Λεπτά:Δευτερόλεπτα (ωω:λλ:δδ)";
$l_unixTime             = "Χρόνος Unix:";

$l_aed = "Ντιρχάμ ΗΑΕ";
$l_ars = "Πέσο Αργεντινής";
$l_aud = "Αυστραλιανό Δολάριο";
$l_bdt = "Τάκα Μπαγκλαντές";
$l_bhd = "Δηνάριο Μπαχρέιν";
$l_bmd = "Δολάριο Βερμούδων";
$l_brl = "Βραζιλιάνικο Ρεάλ";
$l_btc = "Bitcoin";
$l_cad = "Καναδικό Δολάριο";
$l_chf = "Ελβετικό Φράγκο";
$l_clp = "Πέσο Χιλής";
$l_cny = "Ρενμίνμπι";
$l_czk = "Τσεχική Κορώνα";
$l_dkk = "Δανική Κορώνα";
$l_eth = "Ethereum";
$l_eur = "Ευρώ";
$l_gbp = "Λίρα Στερλίνα";
$l_gel = "Λάρι Γεωργίας";
$l_hkd = "Δολάριο Χονγκ Κονγκ";
$l_huf = "Ουγγρικό Φιορίνι";
$l_idr = "Ρουπία Ινδονησίας";
$l_ils = "Νέο Ισραηλινό Σέκελ";
$l_inr = "Ινδική Ρουπία";
$l_jpy = "Γιεν";
$l_krw = "Νοτιοκορεατικό Γουόν";
$l_kwd = "Κουβεϊτιανό Δηνάριο";
$l_lkr = "Ρουπία Σρι Λάνκα";
$l_ltc = "Litecoin";
$l_mmk = "Κυάτ Μιανμάρ";
$l_mxn = "Πέσο Μεξικού";
$l_myr = "Μαλαισιανό Ρίνγκιτ";
$l_ngn = "Νάιρα Νιγηρίας";
$l_nok = "Νορβηγική Κορώνα";
$l_nzd = "Δολάριο Νέας Ζηλανδίας";
$l_php = "Πέσο Φιλιππίνων";
$l_pkr = "Πακιστανική Ρουπία";
$l_pln = "Ζλότυ";
$l_rub = "Ρωσικό Ρούβλι";
$l_sar = "Σαουδαραβικό Ριάλ";
$l_sek = "Σουηδική Κορώνα";
$l_sgd = "Δολάριο Σιγκαπούρης";
$l_thb = "Μπατ";
$l_try = "Τουρκική Λίρα";
$l_twd = "Νέο Δολάριο Ταϊβάν";
$l_uah = "Γρίβνα";
$l_usd = "Δολάριο ΗΠΑ";
$l_vef = "Βενεζουελανικό Μπολιβάρ";
$l_vnd = "Βιετναμέζικο Ντονγκ";
$l_xag = "Ασήμι (τρόι ουγγιά)";
$l_xau = "Χρυσός (τρόι ουγγιά)";
$l_zar = "Νότιο Αφρικανικό Ραντ";

$l_info_main         = "Οι ισοτιμίες σε αυτή την ιστοσελίδα είναι μόνο για ενημερωτικούς σκοπούς.&nbsp;Δεν είναι εγγυημένο ότι είναι ακριβείς και υπόκεινται σε αλλαγές χωρίς προειδοποίηση.";

$l_data_provided     = "Δεδομένα παρέχονται από";
$l_cg_url            = "https://www.coingecko.com/el/coins/monero";
$l_cg_url_hreflang    = "el";
$l_coingecko         = $l_data_provided . " <a class=\"text-white\" href=\"$l_cg_url\" hreflang=\"$l_cg_url_hreflang\" rel=\"external\" target=\"_blank\">CoinGecko</a>.";

$l_getmonero         = '<a class="text-white" href="https://www.getmonero.org/el/" hreflang="el" target="_blank" rel="external">Επίσημος ιστότοπος</a> | <a class="text-white" href="https://ccs.getmonero.org/" hreflang="en" target="_blank" rel="external">CCS</a> | <a class="text-white" href="https://www.monero.observer/resources/" hreflang="en" target="_blank" rel="external">Observer</a> | <a class="text-white" href="https://www.monerotalk.live/" hreflang="en" target="_blank" rel="external">Talk</a>';
$l_countrymonero     = '&nbsp;| <a class="text-white" href="https://t.me/monerogr" hreflang="en" target="_blank" rel="external">Telegram GR</a>';

$l_donation_page     = "σελίδα δωρεών";
$l_kuno_homepage_url = "https://kuno.anne.media/";
$l_kuno_homepage_url_hreflang = "en";
$l_kuno_text         = "Kuno – Συγκέντρωση κεφαλαίων με Monero";
$l_webserver_sponsor = "Φιλοξενία ιστότοπου παρέχεται από";

$l_api_update        = $l_last_at . " <u title=\"$l_hr_min_sec\"><span id=\"lastUpdateDisplay\">00:00:00</span></u>, <span id=\"displayTimeZone\">τοπική ώρα</span>.";

$l_info              = $l_info_main."&nbsp;<span id=\"refreshInfo\" class=\"mb-1\">--</span>&nbsp;$l_api_update&nbsp;$l_coingecko<br><a target=\"_blank\" href=\"https://kuno.anne.media/donate/onml/\" rel=\"external\" hreflang=\"en\"><img loading=\"lazy\" align=\"middle\" src=\"./img/kuno-monero-26x26.png\" width=\"17\" height=\"17\" alt=\"Kuno - Moner.ooo $l_donation_page\"></a>&nbsp;<a target=\"_blank\" href=\"$l_kuno_homepage_url\" class=\"text-white\" rel=\"external\" hreflang=\"$l_kuno_homepage_url_hreflang\">$l_kuno_text</a> | <a class=\"text-white\" href=\"https://github.com/nice42q/moner.ooo\" hreflang=\"en\" rel=\"external\" target=\"_blank\">GitHub</a> | <a style=\"text-decoration:none; font-weight:bold;\" class=\"text-white\" href=\"https://servers.guru/\" hreflang=\"en\" rel=\"external\" target=\"_blank\">$l_webserver_sponsor<img loading=\"lazy\" align=\"middle\" src=\"./img/servers-guru.svg\" height=\"19\" alt=\"Servers Guru\" title=\"Servers Guru\"></a>";