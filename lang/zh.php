<?php
// Translator： Winslow SorenEricMent(逸雩)
$l_lang_meta         = "zh";
$l_rtl               = "false";
$l_page_title        = "门罗币与欧元/BTC/CHF/美元等币种汇率";
$l_meta_description  = "门罗币实时多币种汇率，免费提供。";
$l_meta_keywords     = "门罗币, XMR, 法币, 价格, 汇率, 实时, 交易, 结汇";

$l_title_h1          = "转换至";
$l_fiatSelect        = "选择货币";
$l_fiatInput         = "法币数值输入框";
$l_xmrInput          = "门罗币数值输入框";
$l_clipboard_copy_tooltip = "复制到剪贴板";
$l_copytoclipboard_success_text = "已复制！";
$l_copytoclipboard_error_text   = "错误！";

$l_refresh_info_minute  = "汇率每分钟更新一次。";
$l_refresh_info_minutes = "汇率每 %s 分钟更新一次。";
$l_refresh_info_seconds = "汇率每 %s 秒更新一次。";
$l_last_at              = "最后更新于";
$l_hr_min_sec           = "时:分:秒 (hh:mm:ss)";
$l_unixTime             = "Unix 时间：";

$l_aed = "阿联酋迪拉姆";
$l_ars = "阿根廷比索";
$l_aud = "澳元";
$l_bdt = "孟加拉国塔卡";
$l_bhd = "巴林第纳尔";
$l_bmd = "百慕大元";
$l_brl = "巴西雷亚尔";
$l_btc = "比特币";
$l_cad = "加元";
$l_chf = "瑞士法郎";
$l_clp = "智利比索";
$l_cny = "人民币";
$l_czk = "捷克克朗";
$l_dkk = "丹麦克朗";
$l_eth = "以太坊";
$l_eur = "欧元";
$l_gbp = "英镑";
$l_gel = "格鲁吉亚拉里";
$l_hkd = "港币";
$l_huf = "匈牙利福林";
$l_idr = "印尼盾";
$l_ils = "以色列新谢克尔";
$l_inr = "印度卢比";
$l_jpy = "日元";
$l_krw = "韩元";
$l_kwd = "科威特第纳尔";
$l_lkr = "斯里兰卡卢比";
$l_ltc = "莱特币";
$l_mmk = "缅甸元";
$l_mxn = "墨西哥比索";
$l_myr = "马来西亚林吉特";
$l_ngn = "尼日利亚奈拉";
$l_nok = "挪威克朗";
$l_nzd = "新西兰元";
$l_php = "菲律宾比索";
$l_pkr = "巴基斯坦卢比";
$l_pln = "兹罗提";
$l_rub = "俄罗斯卢布";
$l_sar = "沙特里亚尔";
$l_sek = "瑞典克朗";
$l_sgd = "新加坡元";
$l_thb = "泰铢";
$l_try = "土耳其里拉";
$l_twd = "新台币";
$l_uah = "格里夫纳";
$l_usd = "美元";
$l_vef = "委内瑞拉玻利瓦尔";
$l_vnd = "越南盾";
$l_xag = "白银（盎司）";
$l_xau = "黄金（盎司）";
$l_zar = "南非兰特";

$l_info_main         = "本站提供的汇率仅供参考。&nbsp;其准确程度不被保证，且可能随时发生变更。";

$l_data_provided     = "数据提供：";
$l_cg_url            = "https://www.coingecko.com/zh/数字货币/门罗币";
$l_cg_url_hreflang    = "zh";
$l_coingecko         = $l_data_provided . " <a class=\"text-white\" href=\"$l_cg_url\" hreflang=\"$l_cg_url_hreflang\" rel=\"external\" target=\"_blank\">CoinGecko</a>。";

$l_getmonero         = '<a class="text-white" href="https://www.getmonero.org/zh-cn/" hreflang="zh-cn" target="_blank" rel="external">官方网站</a> | <a class="text-white" href="https://ccs.getmonero.org/" hreflang="en" target="_blank" rel="external">CCS</a> | <a class="text-white" href="https://www.monero.observer/resources/" hreflang="en" target="_blank" rel="external">Observer</a> | <a class="text-white" href="https://www.monerotalk.live/" hreflang="en" target="_blank" rel="external">Talk</a>';
$l_countrymonero     = '&nbsp;| <a class="text-white" href="https://t.me/monerocn" hreflang="en" target="_blank" rel="external">Telegram CN</a>';

$l_donation_page     = "捐赠页面";
$l_kuno_homepage_url = "https://kuno.anne.media/";
$l_kuno_homepage_url_hreflang = "en";
$l_kuno_text         = "Kuno – 使用门罗币筹款";
$l_webserver_sponsor = "网络托管由以下提供：";

$l_api_update        = $l_last_at . " <u title=\"$l_hr_min_sec\"><span id=\"lastUpdateDisplay\">00:00:00</span></u> (<span id=\"displayTimeZone\">当地时间</span>)。";

$l_info              = $l_info_main."&nbsp;<span id=\"refreshInfo\" class=\"mb-1\">--</span>&nbsp;$l_api_update&nbsp;$l_coingecko<br><a target=\"_blank\" href=\"https://kuno.anne.media/donate/onml/\" rel=\"external\" hreflang=\"en\"><img loading=\"lazy\" align=\"middle\" src=\"./img/kuno-monero-26x26.png\" width=\"17\" height=\"17\" alt=\"Kuno - Moner.ooo $l_donation_page\"></a>&nbsp;<a target=\"_blank\" href=\"$l_kuno_homepage_url\" class=\"text-white\" rel=\"external\" hreflang=\"$l_kuno_homepage_url_hreflang\">$l_kuno_text</a> | <a class=\"text-white\" href=\"https://github.com/nice42q/moner.ooo\" hreflang=\"en\" rel=\"external\" target=\"_blank\">GitHub</a> | <a style=\"text-decoration:none; font-weight:bold;\" class=\"text-white\" href=\"https://servers.guru/\" hreflang=\"en\" rel=\"external\" target=\"_blank\">$l_webserver_sponsor<img loading=\"lazy\" align=\"middle\" src=\"./img/servers-guru.svg\" height=\"19\" alt=\"Servers Guru\" title=\"Servers Guru\"></a>";