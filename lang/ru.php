<?php
$l_lang_meta         = "ru";
$l_rtl               = "false";
$l_page_title        = "Конвертация XMR в EUR/BTC/CHF/USD и многие другие";
$l_meta_description  = "Актуальный курс обмена Monero на различные валюты, бесплатно для всех.";
$l_meta_keywords     = "Monero, XMR, фиат, стоимость, курс, живой, обмен, конвертация";

$l_title_h1          = "конвертация в";
$l_moneroooTable = "Сервис предоставлен <a href='https://moner.ooo/'>Moner.ooo</a>, данные предоставлены <a href='https://www.coingecko.com/ru/coins/monero' hreflang='ru' rel='external'>CoinGecko</a>";
$l_fiatSelect        = "Выбор валюты";
$l_fiatInput         = "Поле ввода значения в фиатной валюте";
$l_xmrInput          = "Поле ввода значения в Monero";
$l_clipboard_copy_tooltip = "Копировать в буфер обмена";
$l_copytoclipboard_success_text = "Скопировано!";
$l_copytoclipboard_error_text   = "Ошибка!";

$l_refresh_info_minute  = "Курсы обновляются каждую минуту.";
$l_refresh_info_minutes = "Курсы обновляются каждые %s минут.";
$l_refresh_info_seconds = "Курсы обновляются каждые %s секунд.";
$l_last_at              = "Последнее обновление:";
$l_hr_min_sec           = "Часы:Минуты:Секунды (чч:мм:сс)";
$l_unixTime             = "Время Unix:";

$l_aed = "Дирхам ОАЭ";
$l_ars = "Аргентинское песо";
$l_aud = "Австралийский доллар";
$l_bdt = "Бангладешская така";
$l_bhd = "Бахрейнский динар";
$l_bmd = "Бермудский доллар";
$l_brl = "Бразильский реал";
$l_btc = "Биткоин";
$l_cad = "Канадский доллар";
$l_chf = "Швейцарский франк";
$l_clp = "Чилийское песо";
$l_cny = "Китайский юань";
$l_czk = "Чешская крона";
$l_dkk = "Датская крона";
$l_eth = "Ethereum";
$l_eur = "Евро";
$l_gbp = "Фунт стерлингов";
$l_gel = "Грузинский лари";
$l_hkd = "Гонконгский доллар";
$l_huf = "Венгерский форинт";
$l_idr = "Индонезийская рупия";
$l_ils = "Новый израильский шекель";
$l_inr = "Индийская рупия";
$l_jpy = "Иена";
$l_krw = "Южнокорейская вона";
$l_kwd = "Кувейтский динар";
$l_lkr = "Шри-ланкийская рупия";
$l_ltc = "Litecoin";
$l_mmk = "Мьянманский кят";
$l_mxn = "Мексиканское песо";
$l_myr = "Малайзийский ринггит";
$l_ngn = "Нигерийская найра";
$l_nok = "Норвежская крона";
$l_nzd = "Новозеландский доллар";
$l_php = "Филиппинское песо";
$l_pkr = "Пакистанская рупия";
$l_pln = "Польский злотый";
$l_rub = "Российский рубль";
$l_sar = "Саудовский риял";
$l_sek = "Шведская крона";
$l_sgd = "Сингапурский доллар";
$l_thb = "Тайский бат";
$l_try = "Турецкая лира";
$l_twd = "Новый тайваньский доллар";
$l_uah = "Гривна";
$l_usd = "Доллар США";
$l_vef = "Венесуэльский боливар";
$l_vnd = "Вьетнамский донг";
$l_xag = "Серебро (тройская унция)";
$l_xau = "Золото (тройская унция)";
$l_zar = "Южноафриканский ранд";

$l_info_main         = "Курсы обмена на этом сайте носят ознакомительный характер.&nbsp;Точность не гарантируется, возможны изменения без уведомления.";

$l_data_provided     = "Данные предоставлены";
$l_cg_url            = "https://www.coingecko.com/ru/Криптовалюты/Монеро";
$l_cg_url_hreflang    = "ru";
$l_coingecko         = $l_data_provided . " <a class=\"text-white\" href=\"$l_cg_url\" hreflang=\"$l_cg_url_hreflang\" rel=\"external\" target=\"_blank\">CoinGecko</a>.";

$l_getmonero         = '<a class="text-white" href="https://www.getmonero.org/ru/" hreflang="ru" target="_blank" rel="external">Официальный сайт</a> | <a class="text-white" href="https://ccs.getmonero.org/" hreflang="en" target="_blank" rel="external">CCS</a> | <a class="text-white" href="https://www.monero.observer/resources/" hreflang="en" target="_blank" rel="external">Observer</a> | <a class="text-white" href="https://www.monerotalk.live/" hreflang="en" target="_blank" rel="external">Talk</a>';
$l_countrymonero     = '&nbsp;| <a class="text-white" href="https://t.me/xmr_ru" hreflang="ru" target="_blank" rel="external">Telegram RU</a>';

$l_donation_page     = "страница пожертвований";
$l_kuno_homepage_url = "https://kuno.anne.media/";
$l_kuno_homepage_url_hreflang = "en";
$l_kuno_text         = "Kuno – Сбор средств в Monero";
$l_webserver_sponsor = "Веб-хостинг предоставлен";

$l_api_update        = $l_last_at . " <u title=\"$l_hr_min_sec\"><span id=\"lastUpdateDisplay\">00:00:00</span></u> (<span id=\"displayTimeZone\">местное время</span>).";

$l_info              = $l_info_main."&nbsp;<span id=\"refreshInfo\" class=\"mb-1\">--</span>&nbsp;$l_api_update&nbsp;$l_coingecko<br><a target=\"_blank\" href=\"https://kuno.anne.media/donate/onml/\" rel=\"external\" hreflang=\"en\"><img loading=\"lazy\" align=\"middle\" src=\"./img/kuno-monero-26x26.png\" width=\"17\" height=\"17\" alt=\"Kuno - Moner.ooo $l_donation_page\"></a>&nbsp;<a target=\"_blank\" href=\"$l_kuno_homepage_url\" class=\"text-white\" rel=\"external\" hreflang=\"$l_kuno_homepage_url_hreflang\">$l_kuno_text</a> | <a class=\"text-white\" href=\"https://github.com/nice42q/moner.ooo\" hreflang=\"en\" rel=\"external\" target=\"_blank\">GitHub</a> | <a style=\"text-decoration:none; font-weight:bold;\" class=\"text-white\" href=\"https://servers.guru/\" hreflang=\"en\" rel=\"external\" target=\"_blank\">$l_webserver_sponsor<img loading=\"lazy\" align=\"middle\" src=\"./img/servers-guru.svg\" height=\"19\" alt=\"Servers Guru\" title=\"Servers Guru\"></a>";